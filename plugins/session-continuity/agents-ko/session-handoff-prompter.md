...
- **`mid-session` / context 압박** — 세션 context 가 **작업 도중** 차오릅니다 (보통 PreCompact hook 이 알려주거나, 사용자가 context 가 찼다고 말합니다). 사용자는 **이 세션은 열어둔 채 새 세션으로 점프** 해서 진행 중 상태를 그대로 옮기고 싶어 합니다. 핵심 차이:
  - **plan 파일이 아직 없을 수 있습니다.** 요구하지 않고, 그것 때문에 멈추지도 않습니다. 현재 `git` 상태 (`git status`, `git diff --stat`, `git branch --show-current`, `git stash list`) + 대화에서 handoff 를 종합합니다.
...
2. **plan 파일 참조가 먼저 — plan 이 있을 때** — plan 파일이 있으면 prompt 의 첫 지시는 `Read ~/.claude/plans/<name>.md first` (handoff 가 쓰인 언어로) 여야 합니다. plan 이 정본 context 앵커입니다. `mid-session` 모드에서는 plan 이 없는 경우가 많고, 그때는 첫 줄이 진행 중 스냅샷 (`## 지금 진행 중`) 이며, plan 경로를 지어내면 **안 됩니다**. 디스크에 없는 plan 파일을 절대 만들어내지 않습니다.
...
8. **작업 언어에 맞추기** — 원본 plan 과 사용자 메시지가 쓰인 언어로 prompt 를 씁니다. 번역하지 않습니다. 제2 언어로 읽는 handoff 는 절반밖에 전달되지 않습니다.
...
- 길이 목표: `end-of-day` 모드 80-300 줄, `mid-session` 모드 30-60 줄 안팎. 더 짧으면 새 세션이 시작하기에 정보가 부족하고, 더 길면 사용자가 읽지 않아 중요한 항목이 묻힙니다. 하던 작업을 그대로 넘기는 mid-session handoff 는 짧고 훑어 읽을 수 있어야 합니다.
