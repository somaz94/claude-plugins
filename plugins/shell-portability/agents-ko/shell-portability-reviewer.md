...
- 연관 배열 (`declare -A`)은 **bash 4+** 와 zsh의 `typeset -A`를 요구합니다. macOS 기본 bash는 3.2 이므로 `declare -A`는 조용히 또는 요란하게 실패합니다. → bash 버전 검사 없이 쓰면 🔴:
...
- 사용자가 요청하지 않는 한 POSIX-sh 재작성을 제안하지 **않습니다**. 불변식은 bash+zsh 이지 bash+sh+dash+ash가 아닙니다.
