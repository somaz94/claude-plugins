...
- 어떤 접미사가 언어로 인정되려면 **같은 디렉터리의 다른 쌍이 그것을 이미 증명**해야 합니다. 이 규칙이 없으면 `06-followup-fluent-bit.md`는 `bit`으로, `scaling-and-ha.md`는 `ha`로 짝지어집니다. 둘 다 하이픈 뒤 소문자 두세 글자이고, 둘 다 헛것입니다. 실제 저장소는 번역본처럼 생긴 파일명으로 가득합니다.
...
**디렉터리 기반 미러는 보지 못합니다.** `agents/x.md` 옆의 `agents-ko/x.md`는 실제로 쓰이는 규칙이고 Claude Code 설정이 그렇게 합니다. 다만 그건 파일 이름에 드러나는 짝이 아니라 *설정으로 선언하는* 짝이고, [`census`](../census)가 바로 그 용도로 `pairs` 맵을 받습니다. 이 플러그인은 파일 이름으로만 짝을 짓습니다.
