...
`description`은 디스크에 줄바꿈 없는 한 줄로 저장되고, 오래 자란 것은 2,000자를 넘깁니다. 그대로 뿌리면 다음 행을 파묻는 텍스트 덩어리가 됩니다.

그래서 뷰어는 문장이 끝나는 지점마다 줄바꿈을 넣습니다 — 텍스트 자체는 건드리지 않으며, 이것이 유일한 재구성입니다 — 그리고 접힌 행은 세 줄로 자릅니다. 펼치면 전문이 보입니다.
