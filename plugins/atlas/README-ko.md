...
| `path:hooks` · `origin:atlas` | 어디에 사는지, 또는 어느 플러그인이 넣었는지로 |
...
그 데이터를 훑는 방식은 매번 다시 읽는 대신 색인을 씁니다. 항목마다 소문자로 내린 텍스트를 처음 검색할 때 한 번만 만들어 두고, 키 입력마다 모든 항목의 이름·설명·경로·본문·번역본을 다시 이어 붙이지 않습니다. 렌더도 묶어서 처리하므로 여섯 글자 단어를 치면 목록을 여섯 번이 아니라 한 번 다시 그립니다.
