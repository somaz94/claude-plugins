...
더 쉬운 두 가지 답을 일부러 피한 결과입니다. 숨겨 버리면 에이전트를 한국어로만 써 온 트리가 에이전트 0개로 보고됩니다. 평범한 항목으로 나열하면 세션이 닿을 수 없는 것을 닿는다고 말하는 셈이고, 애초에 지불하지 않는 컨텍스트 예산에 그 설명을 청구하게 됩니다. 그래서 표시는 하되 **Issues only**에서 걸러지게 두고, 비용은 0으로 잡습니다.
