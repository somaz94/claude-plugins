...
사용자 수준의 `~/.claude/`에 더해 저장소마다 `.claude/` 디렉터리가 생기고 나면, 답하기 어려워지는 질문이 세 가지 있습니다. 그리고 이 질문들에 답해주는 기본 기능은 없습니다. `census`가 답하되, 아무것도 바꾸지 않습니다.
...
| `/census:portability` | 이 중 내가 아닌 사람에게도 동작할 것은 무엇인가? |

<br/>

## 사용법

Claude Code 세션에서 스킬 이름을 입력하면 됩니다. 스킬이 설정된 루트 전체에 번들 스크립트를 돌리고 그 결과를 읽어서 설명해 줍니다. 스캔과 렌더링은 스크립트가, 해석은 스킬이 맡습니다.

아래 예시는 모두 아래의 작은 설정 하나에서 나온 것입니다. 숫자의 의미를 알 수 있도록 먼저 구조를 보입니다.
...
전체 루트 합계가 아니라 **세션당** 수치를 보세요. 실제로 손댈 지점은 맨 아래 컨텍스트 예산 표입니다. `description` 길이 순으로 정렬되는데, 그게 매 세션 실제로 지불하는 비용이기 때문입니다.
...
### `/census:portability` — 이 중 남에게도 동작할 것은 무엇인가
...
마커 표를 먼저 확인하세요. 아래의 모든 등급이 그 문자열들에서 파생되므로, 등급이 이상하다면 대개 판정이 틀린 게 아니라 마커가 빠진 것입니다. 각 히트는 파일과 줄 번호, 그리고 그 줄의 어디에 걸렸는지까지 인용하므로 아무것도 다시 읽지 않고 판정을 확인할 수 있습니다.

<br/>

### 처음 실행할 때

첫 보고서가 신호가 될지 잡음이 될지는 두 가지 설정에서 갈립니다.

**`projectRoots`**의 기본값은 `["."]`, 즉 현재 디렉터리뿐입니다. 이 상태에서는 전역 루트와, 실행한 디렉터리에 마침 있는 `.claude/`만 잡히고 나머지는 잡히지 않습니다. 저장소들이 실제로 있는 위치(`["~/code/*"]` 같은 값)를 지정하세요.
...
가장 쓸모 있는 출력은 다른 어디서도 드러나지 않는 것, 바로 **상시 점유 컨텍스트 비용**입니다. 모든 에이전트·커맨드·스킬의 `name`과 `description`은 *매* 세션의 시스템 프롬프트에 상주하고, 본문은 호출할 때만 로드됩니다. `claude plugin details`는 이걸 플러그인 하나에 대해서만 보고하고, 설정 전체에 대해 보고해 주는 도구는 없습니다.

비용은 **세션 단위로** 보고합니다. 실제로 치르는 형태가 그것뿐이기 때문입니다. 한 세션은 전역 루트와 시작한 저장소 하나만 로드하지, 모든 저장소를 한꺼번에 로드하지 않습니다. 그래서 전체 루트를 더한 값은 존재하지 않는 세션을 묘사하는 셈입니다. 누적 합계도 그것이 무엇인지 명시해서 함께 보여줍니다.

미러된 저장소들에 걸친 바이트 단위 동일 사본은 하나로 셉니다. 파일은 둘, 자산은 하나입니다.

위의 작은 예시가 아니라, 한동안 자라난 설정의 규모에서는 이렇게 나옵니다.
...
**번역 쌍.** 미러는 보통 원본과 일정한 차이를 두고 갈립니다. 번역본임을 알리는 배너를 앞에 붙이거나, 실제 정의로 로드되지 않도록 frontmatter를 코드 블록으로 감싸는 식의 하우스 스타일이 그렇습니다. 구조를 단순 비교하면 이 관례가 파일마다 한 번씩 보고돼 정작 진짜 드리프트를 묻어버립니다.

그래서 오프셋을 **미러 디렉터리 단위로 보정**합니다. 한 디렉터리의 쌍들이 대체로 공유하는 차이를 그 디렉터리의 기준선으로 삼고, 자기 형제들에게서 벗어나는 파일만 보고합니다. 어떤 하우스 스타일도 하드코딩하지 않으므로, 이 도구가 한 번도 본 적 없는 관례도 똑같이 보정됩니다. 벗어남은 양방향으로 셉니다. 형제 열다섯 개가 모두 가진 배너를 혼자 빠뜨린 파일은, 혼자 섹션을 추가한 파일만큼이나 이상치입니다.

다만 구조가 전부는 아닙니다. 두 파일의 구조가 일치하면서도 미러가 몇 달 뒤처져 있을 수 있고, 내용으로는 판정할 수 없습니다. 번역은 애초에 다르게 읽히도록 *되어 있어서*, 내용을 비교하면 번역 자체가 드리프트로 보고됩니다. 대신 git 이력이 답합니다. 미러가 마지막으로 커밋된 뒤에 원본이 커밋됐다면 미러가 뒤처진 것이고, 이건 어느 언어로 쓰였든 성립합니다. 추적되지 않거나 아직 커밋되지 않은 파일은 틀린 판정 대신 판정 없음으로 둡니다.

**훅.** 훅은 포인터이고, 가리키는 파일은 없을 수 있습니다. 등록은 `settings.json`에 그대로 남아 있으므로 훅은 설정된 것처럼 보이면서 아무 일도 하지 않습니다. 이걸 보고해 주는 도구는 달리 없습니다.
...
모든 항목을 한 대의 머신에 얼마나 강하게 묶여 있는지로 등급을 매깁니다. 훅은 그것이 실행하는 스크립트를 통해 평가합니다. 등록은 포인터일 뿐이고, 하드코딩된 경로는 대상 파일 안에 있기 때문입니다.

마커는 **하드코딩이 아니라 파생**됩니다. `$USER`에서, 저장소를 정리하려고 직접 만든 디렉터리 이름에서, 그리고 스캔 대상 저장소들의 git 원격에서 나옵니다. 공개 포지(forge)에서는 계정 이름이 소유자를 식별하지만, 자체 호스팅 포지에서는 "owner"가 그저 그룹 이름이고 흔한 단어인 경우가 많아 대신 호스트명을 취합니다. 덕분에 같은 검사가 당신의 식별자 대신 다른 사람의 식별자를 잡아낼 수 있습니다.

각 저장소 자신의 이름도 마커지만, **그 저장소 안에서만** 그렇습니다. 저장소 범위 에이전트는 대개 자기 저장소를 상대 경로로 부릅니다. `Reviews changes inside acme-platform/storage/` 같은 서술에는 머신 전역 식별자가 하나도 없어서, 가장 이식성이 낮은 축에 속하면서도 완벽하게 이식 가능한 것으로 채점되곤 합니다. 전역에 적용하면 `docs` 같은 이름의 저장소가 온갖 문장에 걸리지만, 자기 저장소 항목에만 한정하면 걸렸다는 사실이 곧 의미가 됩니다.
...
frontmatter 쪽이 중요한 경우입니다. 특정 저장소를 지목하는 `description`은 Claude가 라우팅에 쓰는 값이므로, 그걸 바꾸면 *언제 이 항목이 발동하는지*가 바뀝니다. 설정 변경이 아니라 재작성입니다.

이식 가능 개수가 적게 나오는 건 성숙한 개인 설정에서 정상이며 결함이 아닙니다. 그 항목들이 상상 속의 일반적인 환경이 아니라 실제 환경을 위해 쓰였다는 뜻입니다.

> **사각지대: 고유명으로 된 인프라.** 마커는 원격과 루트에서 나오므로 클러스터·환경·프로젝트 코드명 같은 이름은 어디서도 공급되지 않습니다. 저장소가 아니라 `prod-eu-1`에 묶인 항목은 여전히 🟢으로 나옵니다. 🟢 목록은 그대로 믿지 말고 한 번 읽어보고, 그런 문자열은 `portability.markers`에 넣으세요.

> **비밀 정보 스캐너가 아닙니다.** 이 도구는 "이게 남에게도 동작할까?"에 답하지, "이게 무언가를 유출할까?"에는 답하지 않습니다. 저장소 레이아웃 이름은 이식성이 없지만 무해하고, 비밀번호는 유출이지만 이식성은 완벽합니다. 공개를 앞두고 있다면 두 검사가 모두 필요합니다.
...
탐색 순서는 다음과 같고, 먼저 발견된 것이 그대로 이깁니다. 파일들을 병합하지 않습니다.
...
| `excludeOssForks` | `upstream` 원격이 있는 저장소는 건너뜀 — 포크에 담긴 `.claude/`는 내 것이 아니라 원 관리자들의 것 |
...
`--out FILE`은 파일로 쓰고, `--json`은 원본 리포트를 내보내며, `--config PATH`는 설정 파일을 지정합니다.
