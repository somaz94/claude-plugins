...
구조는 **펜스 코드블록 바깥만** 셉니다. 펜스 안에 있는 건 구조가 아니라 예시입니다. 셸 블록의 `# 주석`은 heading이 아니고, 예시 출력 블록의 `|`는 표 행이 아닙니다. 이것까지 세면 자기 커맨드라인을 문서화하는 항목은 — 대부분이 그렇습니다 — 충실하게 번역된 미러와도 다르게 보였습니다. 양쪽 예시에 붙은 주석이 서로 다르기 때문입니다.
...
이 마켓플레이스의 플러그인은 각자 독립적으로 버전을 매기고 릴리스합니다. `census`의 모든 변경 이력은 이 디렉터리 커밋만 담아 [census 릴리스](https://github.com/somaz94/claude-plugins/releases?q=census&expanded=true)에 있습니다.
