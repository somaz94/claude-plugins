...
사용자 수준의 `~/.claude/`에 더해 저장소마다 `.claude/` 디렉터리를 두기 시작하면, 답하기 어려워지는 질문이 세 가지 생깁니다. 그런데 이 질문에 답해 주는 기본 기능은 없습니다. `census`가 답하되, 아무것도 바꾸지 않습니다.
...
**`pairs`**는 기본값이 비어 있고, 이 상태에서는 번역 쌍 검사가 통째로 꺼집니다. 미러를 두고 있다면 `{"agents": "agents-ko"}`처럼 이름을 지정하세요. 그때부터 `/census:drift`가 누락됐거나 모양이 어긋났거나 원본보다 뒤처진 미러를 보고하기 시작합니다. 두 설정 모두 아래 설정 파일에서 지정합니다.
...
구조는 **펜스 코드블록 바깥만** 셉니다. 펜스 안에 있는 건 구조가 아니라 예시입니다. 셸 블록의 `# 주석`은 제목이 아니고, 예시 출력 블록의 `|`는 표 행이 아닙니다. 이것까지 세면 자기 커맨드라인을 문서화하는 항목(대부분이 그렇습니다)은 충실하게 번역된 미러와도 달라 보였습니다. 양쪽 예시에 붙은 주석이 서로 다르기 때문입니다.
...
마커는 **하드코딩된 값이 아니라 파생된 값**입니다. `$USER`에서, 저장소를 정리하려고 직접 만든 디렉터리 이름에서, 그리고 스캔 대상 저장소들의 git 원격에서 나옵니다. 공개 포지(forge)에서는 계정 이름이 소유자를 식별하지만, 자체 호스팅 포지에서는 "owner"가 그저 그룹 이름이고 흔한 단어인 경우가 많아 대신 호스트명을 씁니다. 덕분에 같은 검사가 내 식별자 대신 다른 사람의 식별자를 잡아낼 수 있습니다.
...
**user 수준** 항목은 그 반대 경우라, 규칙도 정반대여야 합니다. 어느 저장소에도 속하지 않아 위 규칙이 닿지 않는데, 정작 특정 저장소 몇 개를 두고 쓰인 항목이 바로 이쪽입니다(`Runs the migration suite for acme-billing-api`). 그래서 `projectRoots`가 가리키는 모든 저장소 이름이 user 수준 항목의 마커가 됩니다. 그 저장소가 자체 `.claude/`를 갖고 있는지는 상관없습니다 — 저장소는 존재한다는 사실만으로 이 머신을 식별합니다. 단, **여러 토큰으로 된 이름만** 해당합니다(`acme-billing-api`는 되고 `docs`는 안 됩니다). 이 조건이 흔한 단어가 마커로 잘못 걸리는 문제를 막습니다.

<br/>

## 공유 가능과 이식 가능은 다릅니다

등급은 파일 하나를 설명합니다. 하지만 무언가를 배포하는 건 파일 하나만 보고 답할 질문이 아닙니다.
...
참조는 백틱 안에서만 찾습니다. 위임하는 항목은 모두 대상을 그렇게 표기하고, 코드 스팬 안으로 한정해 두면 `release`나 `scan` 같은 짧은 이름이 평범한 문장에 걸리지 않습니다. 여기서는 참조를 놓치는 쪽이 없는 참조를 지어내는 쪽보다 비용이 훨씬 적습니다.
...
> **사각지대: 백틱 없이 언급된 의존 대상.** 참조는 코드 스팬 안에서만 탐지합니다. *셸 리뷰어에게 넘긴다*처럼 백틱 없이 일반 문장으로 쓴 항목은 실제 의존이 있어도 어떤 보고서에도 나오지 않습니다. 의도한 선택입니다. 문장까지 뒤지면 찾아내는 것보다 지어내는 것이 훨씬 많아지기 때문입니다. 그래서 막히지 않은 🟢은 증거일 뿐 증명은 아닙니다.
...
| `pairs` | 번역 미러 디렉터리, 미러 대상 디렉터리를 키로 지정. 비우면 번역 쌍 검사가 꺼짐 |
...
이 마켓플레이스의 플러그인은 각자 독립적으로 버전을 매기고 릴리스합니다. `census`의 모든 변경 이력은 이 디렉터리의 커밋만 모은 [census 릴리스](https://github.com/somaz94/claude-plugins/releases?q=census&expanded=true)에서 볼 수 있습니다.
